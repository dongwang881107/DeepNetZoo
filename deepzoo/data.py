...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        pet10, pet60 = self.pet10[idx], self.pet60[idx]
        if self.transform:
            pet10, pet60 = self.transform(pet10, pet60)
        if (self.patch_size!=None) & (self.patch_n!=None):
            pet10, pet60 = self.get_patch(pet10, pet60)
        else:
            pet10 = pet10.unsqueeze(0)
            pet60 = pet60.unsqueeze(0)
        return (pet10, pet60)
    ...
    def get_patch(self, pet10, pet60):
        ...
        pet10_patch = torch.Tensor(self.patch_n, self.patch_size, self.patch_size)
        ...
        while i < self.patch_n:
            ...
            if torch.max(pet60_p) > torch.tensor([1e-3]):
                pet10_patch[i,:,:] = pet10_p
                pet60_patch[i,:,:] = pet60_p
                i += 1
        return pet10_patch, pet60_patch
